	...
	function get_pagetitle($bef_title){
		...
		switch($action){
			case "vforum":
				...
				break;
			case "viewforum":
				$title = $default_title.$this->get_groupname($this->forum_get_parent($this->check_parms($_GET['f'])))." &raquo; ".$this->get_forumname($this->check_parms($_GET['f']));
				break;
			case "viewtopic":
				$group = $this->get_groupname($this->forum_get_parent($this->forum_get_forum_from_post($this->check_parms($_GET['t']))));
				$title = $default_title.$group." &raquo; ".$this->get_forumname($this->forum_get_forum_from_post($this->check_parms($_GET['t'])))." &raquo; ".$this->get_threadname($this->check_parms($_GET['t']));
				break;
			case "search":
	...
	global $wpdb;
		...
		foreach ($parents as $parent) {
			if ($this->options['forum_use_seo_friendly_urls'])
			{
				...
				$trail .= " <strong>&raquo;</strong> <a href='".rtrim($this->home_url, '/').'/'.$group.'/'.$forum.".0'>".$this->get_forumname($parent)."</a>";
			}
			else
				$trail .= " <strong>&raquo;</strong> <a href='$this->base_url"."viewforum&f=$parent.0'>".$this->get_forumname($parent)."</a>";
		}
